...
class PredictionHelper(
    ...
    fun predict(inputArray: Array<String>) {
        executorService.execute {
            try {
                val outputArray = Array(1) { arrayOf("") } // Output array for single output

                interpreter?.run(inputArray, outputArray)

                // Assuming the model returns a single output
                val outputResult = outputArray[0][0]
